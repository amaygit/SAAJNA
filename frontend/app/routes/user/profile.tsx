...
const Profile = () => {
  ...
  const handleAvatarChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    formData.append("upload_preset", "user_avatar_preset");

    try {
      const res = await fetch("https://api.cloudinary.com/v1_1/dsybwbdbc/image/upload", {
        method: "POST",
        body: formData,
      });
  ...
  const handlePasswordChange = (values: ChangePasswordFormData) => {
    changePassword(values, {
      onSuccess: () => {
        toast.success("Password updated. You will be logged out shortly.");
        form.reset();
        setTimeout(() => {
          logout();
          navigate("/sign-in");
